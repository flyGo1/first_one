    ...
    for cnt in contours:  # 每一个轮廓线      hierarchy是层次关系，比如一个轮廓在另一个里面。。
        ...
        if area > minArea:  # 面积大于800像素为封闭图形
            ...
            if w <= h:  # 得到一个正方形框，边界往外扩充10像素,黑色边框
                ...
                yy = y-10
                ss = h+20
                cv2.rectangle(imgCopy, (x-dd-10, y-10), (x+a+10, y+h+10), (255 , 0 , 0), 3)    # 看看框选的效果，在imgCopy中
                print(a+dd, h)
            else:               # 边界往外扩充10像素值
                ...
                yy = y-dd-10
                ss = w+20
                cv2.rectangle(imgCopy, (x-10, y-dd-10), (x+w+10, y+a+10), (255, 0, 0), 3)
                print(a+dd, w)
    ...
    if len(Borderlist) != 0:    # 不能为空
        for (imgRes,x,y,s) in Borderlist:
            if x >= 10 and y >= 10 and s > 15:
                ...
                #img原始图像
                #imgCopy框选数字并识别的图像
                #imgProcess处理后的黑底白字图像
                #imgRes单个数字图像

                #stackImg = stackImages(0.6, ([img, imgCopy], [imgProcess, imgRes]))
                stackImg = stackImages(0.6,([imgCopy], [imgProcess]))
